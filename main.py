...
def embed_jangmun_end(syl):
    words = ENDWORDS.get(syl, [])
    if not words:
        return discord.Embed(title=f"-{syl} 로 끝나는 단어 없음", color=0x9AA4B2)
    words = sorted(words, key=lambda w: (-len(w), w))[:30]   # 긴 것부터 30개
    lines = [f"**{i+1}.** {w}  `{len(w)}자`" for i, w in enumerate(words)]
    return discord.Embed(title=f"📏🏁  '-{syl}' 로 끝나는 최장 단어 TOP {len(words)}",
                         description="\n".join(lines), color=0x00C2A8)
...
async def on_message(msg):
    ...
    if c.startswith("!공격"):
        ...
        s, err = first_syllable(c[len("!장문종결"):])
        if err: await msg.channel.send(err)
        else:   await msg.channel.send(embed=embed_jangmun_end(s))
