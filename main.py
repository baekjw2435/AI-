...
def embed_jonggyeol(syl):
    words = ENDWORDS.get(syl, [])
    if not words:
        return discord.Embed(title=f"-{syl} 로 끝나는 단어 없음", color=0x9AA4B2)
    words = sorted(words, key=lambda w: (len(w), w))   # 짧은 것부터
    return discord.Embed(title=f"🏁  '-{syl}' 로 끝나는 단어 · {len(words)}개",
                         description=join_cap(words, 3600), color=0x00C2A8)
...
async def on_message(msg):
    ...
    if c.startswith("!공격"):
        ...
        if err: await msg.channel.send(err)
        else:   await msg.channel.send(embed=embed_jonggyeol(s))
